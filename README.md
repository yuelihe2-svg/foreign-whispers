...
The Military Drones video was evaluated during development but was not selected as a final submitted demo artifact because it contains more than 1300 translated segments, making full Chatterbox TTS synthesis computationally expensive. The same pipeline supports this video, but the final submission focuses on three shorter completed clips that are easier to reproduce and verify.
...
The submitted sample output package contains three completed demo videos:
...
For each demo video, the sample package includes the original input MP4, the Spanish dubbed output MP4, translated WebVTT captions, Whisper transcription JSON, Argos translation JSON, diarization JSON, and a TTS alignment report.


The primary sample output is:

    final_submission/sample_outputs/rob_reiner/output_dubbed_es.mp4


The generated output videos preserve the original visual stream, replace the original audio with generated Spanish TTS audio, and provide translated WebVTT captions through the API/frontend.
...
The submitted code can run the full pipeline through the local Docker stack when the machine has a compatible NVIDIA GPU. For the submitted demo artifacts, some of the heavier Chatterbox TTS generation jobs were optionally run on an HPC GPU machine to improve stability and reduce waiting time. This does not change the application design; the same pipeline can run through the local Docker stack on a suitable NVIDIA GPU. The generated WAV artifacts were then copied back into the project’s `pipeline_data/api/tts_audio/chatterbox/` directory and stitched locally.
...
5. Sample output artifacts for three completed demo videos.
